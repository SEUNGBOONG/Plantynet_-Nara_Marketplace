...
# 🌟 조달청 500 에러를 방지하기 위해 서버 검색 파라미터를 과감히 제거하고, 파이썬이 직접 필터링하도록 설계!
API_SOURCES = [
    {
...
def _fetch_source_items(source, start_dt, end_dt, service_key):
    # 🌟 조달청에 검색 키워드를 안 보내고 전체 내역을 안전하게 요청!
    params = {
    ...
    for source in API_SOURCES:
        for start_dt, end_dt in date_chunks:
            ...
            for item in items:
                ...
                # 🌟 조달청 서버 대신 우리 로봇이 내부에서 완벽하게 키워드 필터링!
...
def send_all(new_items, ongoing_items):
    ...
    if TEAMS_WEBHOOK_URL:
        try: requests.post(TEAMS_WEBHOOK_URL, json={"text": messenger_text}, timeout=10)
        except: pass
    if SLACK_TOKEN and SLACK_CHANNEL:
        try: requests.post("https://slack.com/api/chat.postMessage", headers={"Authorization": f"Bearer {SLACK_TOKEN}"}, json={"channel": SLACK_CHANNEL, "text": messenger_text}, timeout=10)
        except: pass
    if NAVER_EMAIL and NAVER_PASSWORD:
        msg = MIMEMultipart()
        msg['From'] = NAVER_EMAIL; msg['To'] = NAVER_EMAIL
        ...
        else: html += "<p style='color: gray;'>오늘 등록된 최신 정보가 없습니다.</p>"
        ...
        else: html += "<tr><td colspan='3' style='text-align:center; color:gray;'>내역이 없습니다.</td></tr>"
        html += "</table>"; msg.attach(MIMEText(html, 'html', 'utf-8'))
        try:
            server = smtplib.SMTP_SSL('smtp.naver.com', 465)
            server.login(NAVER_EMAIL, NAVER_PASSWORD); server.sendmail(NAVER_EMAIL, NAVER_EMAIL, msg.as_string()); server.close()
