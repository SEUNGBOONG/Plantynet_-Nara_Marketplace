...
def fetch_order_plans(endpoint, service_key, params):
    # 🎯 조달청 서버의 한글 깨짐 우회를 위해 safe=""로 철저하게 인코딩 처리
...
def get_date_ranges(kst_now):
    ...
    # 🎯 [06번 에러 해결] PPSSrch 명세 기준에 부합하게 8자리(YYYYMMDD)로 연동
    return {
...
def get_g2b_data():
    ...
    for category, endpoint in ORDER_PLAN_ENDPOINTS:
        for keyword in keywords:
            # 🎯 [11번 에러 해결] PPSSrch의 필수 파라미터 규칙에 맞게 bizNm에 검색어 주입
            params = {
                "type": "json",
                "numOfRows": 100,
                "bizNm": keyword,
...
def send_alerts(items, new_count, errors):
    ...
    if errors and not items:
        ...
        # 🎯 전날에 없던 새로운 공고 발견 시 제목에 NEW 강조 및 건수 표기
